...
The boxplot shows the distribution of tempi of the highest and lowest 5 scoring Eurovision songs of all years. The tempo doesn't seem to differ a lot between groups. We can therefore conclude that tempo is not a distinguishing feature between succesful and unsuccesful Eurovision songs. 
...
Now we have taken a look at the relation between one feature and of a song is succesful or not on Eurovision, we move on to the big work. Is it possible to predict if a song is successful or not on Eurovision based on all it's features? To investigate this, different techniques where used.

**Dendogram**

First, a dendogram was created to see how similar songs were, based on the Spotify track features from one year of Eurovision. The top 5 consisted out of the songs "Think about things", "Fai Rumore", "UNO", "Move" and "YES". The lowest 5 consisted out of "Da Vidna", "FREAKY!", "YOU", "Prison" and "Divli Vietre". It is clear from the dendogram that the top 5 or lowest 5 songs aren't really similar to each other. UNO, which ended third, is very different from the all other songs. "Divli Vietre", which ended in last place, is rather similar to "Fai Rumore", which ended second. The same thing goed for "You" and "Move". This dendogram only shows similarity for one year, but could give some insight in what to inspect when analyzing all years: making predictions on classes that aren't very similar to one another might not give the best results.
